/* =========================================================================
   ...
   GET    /.netlify/functions/items            -> array con todas las presentaciones,
                                                  SIN el html completo de cada una (solo
                                                  metadatos: distrito, kpi, fecha, thumb...)
   GET    /.netlify/functions/items?id=<id>     -> una sola presentacion completa (con html)
   ...
   presentación incluye su HTML completo (varios cientos de KB cada una), así
   que el límite de tamaño de Netlify Functions (payloads sync ~6MB) es la
   restricción práctica por presentación individual — ver MAX_ITEM_BYTES
   abajo. El listado general NO manda ese html: con 17 distritos subiendo un
   informe por semana, mandar todo el historial completo en cada carga del
   panel lo iba dejando cada vez más lento — el panel solo necesita los datos
   livianos para mostrar tarjetas y KPIs, y el html real se pide aparte
   (?id=<id>) unicamente cuando alguien abre, descarga o mueve un informe
   puntual.
   ========================================================================= */
...
const MAX_ITEM_BYTES = 6 * 1024 * 1024; // limite practico de una function sincrona de Netlify

exports.handler = async (event) => {
     // Firma clasica de Lambda: Netlify Blobs no autoconfigura el entorno solo aqui,
     // hay que conectarlo explicitamente antes de pedir cualquier store.
     connectLambda(event);
     ...
     try {
            if (event.httpMethod === "GET") {
                     ...
                     if (id) {
                                ...
                                if (!item) return json(404, { error: "No se encontró esa presentación." });
                                return json(200, item);
                     }
                     // listado liviano: sin "html" (el archivo completo de cada informe) para
              // que cargar el panel sea rapido sin importar cuanto historial haya.
              const light = items.map(({ html, ...rest }) => rest);
                     return json(200, light);
            }
